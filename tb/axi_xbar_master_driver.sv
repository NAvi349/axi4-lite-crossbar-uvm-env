...
class axi_xbar_master_driver extends uvm_driver;
   
  virtual axi_xbar_if xif();
   
  uvm_sequence_item tx;
  axi_master_item m_tx;
  
 `uvm_component_utils(axi_xbar_m_driver)
  
  
  function new(string name = "axi_xbar_m_driver", component parent);
  ...
  function void build_phase (uvm_phase phase);
    ...
    `uvm_config_db(axi_xbar_if)::get("", *, xif, "xif");
  endfunction
  ...
  virtual task drive_slave_address_into_master_input(axi_master_item m_tx);
    
    @(xif.cb_m);
    
    
   `uvm_info(get_full_name(), $sformatf("Driving Address of Slave 2 to Master 0"), UVM_LOW)
    xif.cb_m.s_axi_awaddr  [31:0]  <= 'h4000_0001;
    xif.cb_m.s_axi_awvalid [   0]  <= 'b1;
    ...
    xif.cb_m.s_axi_awvalid [   0]  <= 'b0;
    
   `uvm_info(get_full_name(), $sformatf("Got AW[0] Ready"), UVM_LOW)
   
  endtask
  
  virtual task drive_slave_data_into_master_input(axi_master_item m_tx);
    
   @(xif.cb_m);
    
   `uvm_info(get_full_name(), $sformatf("Driving Data of Slave 2 to Master 0"), UVM_LOW)
    
    xif.cb_m.s_axi_wdata   [31:0]  <= 'h0000_0002;
    xif.cb_m.s_axi_wstrb   [ 3:0]  <= 'hF;
    
    xif.cb_m.s_axi_wvalid  [   0]  <= 'b1;
